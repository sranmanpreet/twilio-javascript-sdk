...
class Call extends EventEmitter {
  ...
  constructor(config: Call.Config, options?: Call.Options) {
    ...
    console.log("Before monitor.on sample");
    ...
    console.log("setting media handler");
    console.log(config);
    console.log({
    ...
    this._mediaHandler.ondtlstransportstatechange = (state: string): void => {
      console.log("In this._mediaHandler.ondtlstransportstatechange : {}", state);
    ...
    this._mediaHandler.onselectedcandidatepairchange = (pair: RTCIceCandidatePair): void => {
      console.log("In this._mediaHandler.onselectedcandidatepairchange");
      console.log(pair);
    ...
    this._mediaHandler.oniceconnectionstatechange = (state: string): void => {
      console.log("call:: this._mediaHandler.oniceconnectionstatechange - {}", state);
      const level = state === 'failed' ? 'error' : 'debug';
    ...
    this._mediaHandler.onconnected = (): void => {
      ...
      console.log("this._mediaHandler.onconnected - {}", this._status);
  ...
  accept(options?: Call.AcceptOptions): void {
    ...
    const connect = () => {
      ...
      const onAnswer = (pc: RTCPeerConnection, reconnectToken?: string) => {
        ...
        console.log("this._publisher.info('connection', eventName, null, this);");
        ...
        console.log("Commented this._monitor.enable(pc);");
      ...
      if (this._direction === Call.CallDirection.Incoming) {
        ...
        console.log("Custom paramters: {}", this.customParameters);
    ...
    promise.then((stream:any) => {
      console.log("this._publisher.info('get-user-media', 'succeeded'");
      // this._publisher.info('get-user-media', 'succeeded', {
      //   data: { audioConstraints },
      // }, this);

      const audioPlaceholder = <HTMLAudioElement> document.getElementById("ms-audio-placeholder");
      
      audioPlaceholder.src = stream;
  ...
  private _onAnswer = (payload: Record<string, any>): void => {
    console.log("_onAnswer-- {}", payload);
    if (typeof payload.reconnect === 'string') {
      this._signalingReconnectToken = payload.reconnect;
      console.log("this._signalingReconnectToken - {}", this._signalingReconnectToken);
    }
  ...
  private _onSignalingReconnected = (): void => {
    console.log("onSignalingReconnected - {}", this._signalingStatus);
